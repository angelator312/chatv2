...
class Mongostore {
  ...
  async conect(urlforconnect) {
    const client = new MongoClient(
      urlforconnect ?? "mongodb://test:test@127.0.0.1/test"
    );
  ...
  async addCD(msg) {
    const i = await this.collection.insertOne(msg);
    msg._id = i.insertedId;
    return msg;
  }
  // async deletekey(key) {
  ...
  // }
  async getCD(id) {
    const data = await this.collection.findOne({ chatId: id, type: "chat" });
    if (!data) {
      return null;
    }
    // delete data._id;
  ...
  async getMsgs(chat_id) {
    const data = await this.collection
      .find({ chat: chat_id }, { sort: { time: "ascending" } })
      .toArray();
    if (data.length === 0) {
      return null;
    }
